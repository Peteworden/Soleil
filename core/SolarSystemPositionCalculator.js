...
export class SolarSystemPositionCalculator {
    ...
    static updateObjectRadecDistanceMagnitude(obj, jd, observerBody, observerPosition, latitude, siderealTime) {
        ...
        if (observerBody == '地球' && latitude && siderealTime) {
            const obsOnEarth = new Cartesian(Math.cos(latitude) * Math.cos(siderealTime) * 6378.14 / 1.49598e8, Math.cos(latitude) * Math.sin(siderealTime) * 6378.14 / 1.49598e8, Math.sin(latitude) * 6378.14 / 1.49598e8);

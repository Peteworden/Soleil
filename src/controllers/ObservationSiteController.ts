...
export class ObservationSiteController {
    // 定義済み観測地のデータ
    private static readonly predefinedSites: Record<string, ObservationSite> = {
        "京大西部構内": {
            observerPlanet: '地球',
            name: '京大西部構内',
            latitude: 35.03,
    ...
    private static currentMarker: any = null;
    private static latMap: number = 0;
    private static lonMap: number = 0;
    ...
    private static setupEventListeners(): void {
        const observationSiteSelect = document.getElementById('observation-site-select') as HTMLSelectElement;
     ...
     * 観測地選択の変更を処理
     */
    private static handleObservationSiteChange(event: Event): void {
     ...
     * 手動座標入力の変更を処理
     */
    private static handleManualCoordinateChange(): void {
        const latInput = document.getElementById('lat') as HTMLInputElement;
        const lonInput = document.getElementById('lon') as HTMLInputElement;
        const nsSelect = document.getElementById('NSCombo') as HTMLSelectElement;
        const ewSelect = document.getElementById('EWCombo') as HTMLSelectElement;
    ...
    private static setObservationSite(site: ObservationSite): void {
     ...
     * 現在地を取得
     */
    private static getCurrentLocation(): void {
        // 取得中メッセージを表示
     ...
     * ステータスメッセージを表示
     */
    private static showStatusMessage(message: string, timeoutMs?: number) {
        let msgDiv = document.getElementById('observation-status-message') as HTMLDivElement;
        if (!msgDiv) {
     ...
     * ステータスメッセージを非表示
     */
    private static hideStatusMessage() {
     ...
     * 地図での選択を表示
     */
    private static showMapSelection(): void {
        // オンライン状態の確認
        if (!navigator.onLine) {
     ...
     * 保存された観測地を読み込み
     */
    private static loadSavedObservationSite(): void {
     ...
     * URLパラメータから観測地を設定
     */
    static setObservationSiteFromURL(lat: number, lon: number): void {
        this.setObservationSite({
            observerPlanet: '地球',
            name: 'カスタム',
            latitude: lat,
     ...
     * 定義済み観測地のリストを取得
     */
    static getPredefinedSites(): Record<string, ObservationSite> {
     ...
     * 新しい観測地を追加
     */
    static addPredefinedSite(name: string, lat: number, lon: number, timezone: number): void {
     ...
     * 観測地を削除
     */
    static removePredefinedSite(name: string): boolean {

...
export class SearchController {
    ...
    private static displaySearchResults(query: string) {
        ...
        if (DataStore.messierData) {
            ...
            for (const messier of DataStore.messierData) {
                if (messier.getName().length == 0) continue;
                ...
                if (messier.getOtherNames() && messier.getOtherNames()!.length > 0 && messier.getOtherNames()![0].length > 0) { // ""だけあるときもある
                    result.title = `${messier.getName()} (${messier.getOtherNames()!.join(', ')})`;
                }
        ...
        if (DataStore.recData) {
            ...
            for (const rec of DataStore.recData) {
                if (rec.getName().length == 0) continue;
        ...
        if (DataStore.sharplessData) {
            ...
            if (queryAsSharpless.length > 0 && 
                ...
                for (const sharpless of DataStore.sharplessData) {
                    ...
                    if (
                        sharpless.getName().toLowerCase() === queryAsSharpless
                    ) {
                        startResults.push(result);
                        continue;
                    }
        ...
        allResults.forEach((result: SearchResult) => {
            ...
            button.addEventListener('click', async () => {
                ...
                if (result.type === 'ngc' || result.type === 'ic') {
                    const object = result.data as NGCObject;
                    if (object) {
                        try {
    ...
    private static async selectSearchResult(position0: EquatorialCoordinates, epoch: 'current' | 'j2000' = 'j2000') {
        ...
        const path_azs: number[] = [];
        const path_alts: number[] = [];

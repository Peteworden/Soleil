...
export class TimeController {
    ...
    private static timeSlider: HTMLInputElement | null = null;
    private static intervalId: number | null = null;

    // 新機能用プロパティ
    private static currentStepMinutes: number = 1;
    private static isPlaying: boolean = false;
    private static playIntervalId: number | null = null;
    ...
    static initialize() {
        ...
        const realTime = document.getElementById('realTime') as HTMLInputElement;
    ...
    private static setupTimeControl() {
        this.timeControl = document.getElementById('timeControl') as HTMLDivElement;
        ...
        if (prevBtn) prevBtn.addEventListener('click', () => this.stepBackward());
        if (nextBtn) nextBtn.addEventListener('click', () => this.stepForward());
        ...
        if (sunriseBtn) sunriseBtn.addEventListener('click', () => this.jumpToSunrise());
        if (sunsetBtn) sunsetBtn.addEventListener('click', () => this.jumpToSunset());
    ...
    // スライダー範囲を初期化（基準点を使用）
    private static initSliderRange() {
        if (!this.timeSlider) return;
    ...
    // 時刻を設定（スライダー範囲もリセット）
    private static setTime(jd: number) {
    ...
    private static setTimeWithoutSliderReset(jd: number) {
        this.setTimeInternal(jd);
        // スライダーの値だけ更新
        if (this.timeSlider) {
    ...
    private static updateSliderValue() {
        if (!this.timeControl) return;
        if (!this.timeSlider) return;
    ...
    static updateControlPanelVisibility() {
        ...
        const allHidden =
            (!this.timeControl || this.timeControl.style.display === 'none') &&

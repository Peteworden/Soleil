...
import { TLEData, SatellitePosition, EquatorialCoordinates, ObservationSite } from '../types/index.js';
...
export class SatelliteCalculator {
    private satellites: Map<string, satellite.SatRec> = new Map();
    private observationSite: ObservationSite;

    constructor(observationSite: ObservationSite) {
     ...
     * TLEデータから人工衛星の位置を計算
     */
    calculateSatellitePosition(tle: TLEData, date: Date): SatellitePosition | null {
        try {
            // TLEをパース
            ...
            // 日時をJulian Dateに変換
            const jd = this.dateToJulianDate(date) as any;

            // 人工衛星の位置を計算
            const positionAndVelocity = satellite.propagate(satrec, jd) as any;
     ...
     * 複数の人工衛星の位置を一括計算
     */
    calculateMultipleSatellites(tles: TLEData[], date: Date): SatellitePosition[] {
     ...
     * ECI座標から赤道座標に変換
     */
    private calculateEquatorialCoordinates(positionEci: any, gmst: number): EquatorialCoordinates {
        // ECI座標を赤道座標に変換
     ...
     * 太陽の位置を計算
     */
    private calculateSunPosition(jd: number): any {
        // 簡易的な太陽位置計算
     ...
     */
    private isSatelliteIlluminated(satellitePos: any, sunPos: any): boolean {
        // 地球の半径（km）
     ...
     * 人工衛星の等級を計算（簡易計算）
     */
    private calculateMagnitude(range: number, illuminated: boolean): number {
        if (!illuminated) {

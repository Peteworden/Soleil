...
export class SolarSystemPositionCalculator {
    static updateAllData(objects: SolarSystemObjectBase[], jd: number, observer: string | Cartesian): void {
        if (objects.length === 0) {
            console.warn('天体データがありません');
            return;
        }

        let observerPlanetObject: SolarSystemObjectBase | undefined;
        if (typeof observer === 'string') {
            observerPlanetObject = objects.find(obj => obj.jpnName === observer);
            if (!observerPlanetObject) {
                console.warn(`観測地の惑星「${observer}」が見つかりません`);
                return;
            }
        } else {
            console.warn('観測地が指定されていません');
            return;
        }
        ...
        if (typeof observer === 'string') {
            ...
            objects.forEach(obj => {
                this.updateObjectRadecDistanceMagnitude(obj, jd, undefined, observer, undefined, undefined);
            });
        }
    }
     ...
     * @returns 
     */
    static oneObjectData(objects0: SolarSystemObjectBase[], objName: string, jd: number, observer: string | Cartesian): SolarSystemObjectBase | undefined {
        if (objects0.length === 0) {
            console.warn('天体データがありません');
            return undefined;
        }

        let target: SolarSystemObjectBase;
        if (objects0.find(obj => obj.jpnName === objName) as SolarSystemObjectBase) {
            target = this.deepCopySolarSystemObject(objects0.find(obj => obj.jpnName === objName) as SolarSystemObjectBase);
        } else {
        ...
        let observerPlanetObject: SolarSystemObjectBase | undefined;
        if (typeof observer === 'string') {
            if (objects0.find(obj => obj.jpnName === observer) as SolarSystemObjectBase) {
                observerPlanetObject = this.deepCopySolarSystemObject(objects0.find(obj => obj.jpnName === observer) as SolarSystemObjectBase);
            } else {
        ...
        let earth: SolarSystemObjectBase;
        if (isMoon(target)) {
     ...
     */
    private static updateObjectRadecDistanceMagnitude(
        obj: SolarSystemObjectBase, jd: number,
        observerBody: string | undefined, observerPosition: Cartesian,
     ...
     */
    private static updateMagnitude(obj: SolarSystemObjectBase, observerBody: string | undefined, observerPosition: Cartesian): void {
    ...
    static calculateXYZ(object: SolarSystemObjectBase, jd: number) {
        if (isSun(object)) {
            ...
            if (isOrbitalObject(object)) {
                const orbit = (object as OrbitalObject).orbit as EllipticOrbitalElements | ParabolicOrHyperbolicOrbitalElements;
                if (orbit.e < 0.99999999999999999) {
                    this.calculateEllipticOrbitPositions(object as MinorObject, jd);
                } else if (orbit.e > 1.0000000000000001) {
                    this.calculateHyperbolicOrbitPositions(object as MinorObject, jd);
                } else {
                    this.calculateParabolicOrbitPositions(object as MinorObject, jd);
                }
            }
        } else {
    ...
    static calculateJupiterMoons(jd: number, jupiter: Planet) {
        ...
        const galileoRaDecs: { [key: string]: RaDec } = {};
        for (const [name, moon] of Object.entries(galileo)) {
            ...
            const moonJupitercenRaDecCartesian = new Cartesian(
                moonJupitercenEclpXyz[0],
                moonJupitercenEclpXyz[1],
                moonJupitercenEclpXyz[2]
            ).rotateX(EPSILON);
    ...
    static calculateSaturnMoons(jd: number, saturn: Planet) {
        ...
        const saturnMoonRaDecs: { [key: string]: RaDec } = {};
        for (const [name, moon] of Object.entries(moons)) {
            ...
            const moonSaturncenRaDecCartesian = new Cartesian(
                moonSaturncenEclpXyz[0],
                moonSaturncenEclpXyz[1],
                moonSaturncenEclpXyz[2]
            ).rotateX(EPSILON);
     ...
     */
    private static deepCopySolarSystemObject(obj: SolarSystemObjectBase): SolarSystemObjectBase {
        // 元のオブジェクトのコンストラクタを使用して新しいインスタンスを作成
        const copy = new (obj.constructor as any)({
            jpnName: obj.jpnName,
        ...
        if (obj instanceof Moon) {
            ...
            (copy as Moon).ws = (obj as Moon).ws;

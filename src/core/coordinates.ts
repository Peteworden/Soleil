...
import { HorizontalCoordinates, CartesianCoordinates, EquatorialCoordinates, ViewState, StarChartConfig, CanvasSize } from '../types/index.js';
import { DeviceOrientationManager } from '../device/deviceOrientation.js';
import { LstLat } from '../types/index.js';
...
class RaDec {
    ...
    toCartesian(distance: number = 1): CartesianCoordinates {
...
export class CoordinateConverter {
    ...
    private centerRA: number;
    private centerDec: number;
    private centerAz: number;
    private centerAlt: number;
    private fieldOfViewRA: number;
    private fieldOfViewDec: number;
    ...
    // 現在の中心座標を動的に取得
    private getCurrentCenter(): { ra: number, dec: number, az: number, alt: number } {
    ...
    // 現在のフィールドオブビューを動的に取得
    private getCurrentFieldOfView(): { ra: number, dec: number } {
        const config = this.getCurrentConfig();
        if (config) {
            return {
    ...
    // 赤道座標から直交座標への変換
    equatorialToCartesian(coords: EquatorialCoordinates, distance: number = 1): CartesianCoordinates {
    ...
    // 直交座標から赤道座標への変換
    cartesianToEquatorial(coords: CartesianCoordinates): EquatorialCoordinates {
    ...
    screenRaDecToScreenXY(raDec: EquatorialCoordinates, canvasSize: CanvasSize, viewState: ViewState): [number, number] {
    ...
    screenRaDecToEquatorial_AEP(screenRaDec: EquatorialCoordinates): EquatorialCoordinates {
    ...
    screenRaDecToHorizontal_View(screenRaDec: EquatorialCoordinates): HorizontalCoordinates {
        if (screenRaDec.ra == 0 && screenRaDec.dec == 0) {
            const center = this.getCurrentCenter();
    ...
    screenRaDecToEquatorial(
        lstLat: LstLat, screenRaDec: EquatorialCoordinates, mode: string,
        orientationData?: { alpha: number, beta: number, gamma: number, webkitCompassHeading: number }
    ): EquatorialCoordinates {
        if (mode == 'AEP') {
    ...
    screenRaDecToEquatorial_View(lstLat: LstLat, screenRaDec: EquatorialCoordinates): EquatorialCoordinates {
        const horizontal = this.screenRaDecToHorizontal_View(screenRaDec);
        return this.horizontalToEquatorial(lstLat, horizontal);
    }
    ...
    horizontalToScreenXY_Live(
        ...
        // 基本的な座標変換
        const center = this.getCurrentCenter();
        const centerHorizontal: HorizontalCoordinates = {
            az: center.az,
            alt: center.alt
        };

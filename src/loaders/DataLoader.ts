...
export class DataLoader {
    ...
    private static async fetchGaiaBinaryData(url: string, encodeStyle: string, magOffset: number): Promise<GaiaData> {
        const response = await fetch(url);
        if (!response.ok) {
            throw new Error(`Failed to load ${url}: ${response.statusText}`);
        }
    ...
    // Gaiaデータの読み込み
    static async loadGaiaData(magnitudeRange: string, encodeStyle: string, magOffset: number): Promise<GaiaData> {
        try {

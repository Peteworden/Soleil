...
import { BayerFlamData, ConstellationBoundaryData, ConstellationData, EquatorialCoordinates, StarName } from '../types/index.js';
...
export class DataLoader {
    ...
    private static async fetchGaiaBinaryData(url: string, encodeStyle: string, magOffset: number): Promise<number[][]> {
        const response = await fetch(url);
        if (!response.ok) {
            throw new Error(`Failed to load ${url}: ${response.statusText}`);
        }
    ...
    // 星座データの読み込み
    static async loadConstellationData(): Promise<ConstellationData[]> {
        return await this.fetchJson('data/constellation.json');
        // return await this.fetchJson('下準備/202505星座線書き換え/constellation_new.json');
    }

    static async loadConstellationBoundariesData(): Promise<ConstellationBoundaryData[]> {
        ...
        const constellationBoundaries: ConstellationBoundaryData[] = new Array(boundaryCount);
    ...
    // 星名データの読み込み
    static async loadStarNames(): Promise<StarName[]> {
        const data = await this.fetchJson<any[]>('data/starnames.json');
        const starNames: StarName[] = [];
    ...
    // メシエ天体データの読み込み
    static async loadMessierData(): Promise<MessierObject[]> {
        const converter = new CoordinateConverter();
        const data = await this.fetchJson<any[]>('data/messier.json');
        const messier: MessierObject[] = [];
        for (const object of data) {
            messier.push(
                new MessierObject(
                    object.name,
                    object.alt_name,
                    [object.name].concat(object.alt_name).concat(object.search_keys ? object.search_keys : []),
                    { ra: converter.rahmToDeg(object.ra), dec: converter.decdmToDeg(object.dec) },
                    object.vmag,
                    object.class,
                    object.image_url || null,
                    object.image_credit || null,
                    object.overlay || undefined,
                    object.description,
                    object.wiki || null
                )
            );
    ...
    // おすすめ天体データの読み込み
    static async loadRecData(): Promise<MessierObject[]> {
        const converter = new CoordinateConverter();
        const data = await this.fetchJson<any[]>('data/rec.json');
        const rec: MessierObject[] = [];
        for (const object of data) {
            let ra, dec;
            ...
            rec.push(
                new MessierObject(
                    object.name,
                    object.alt_name,
                    [object.name].concat(object.alt_name).concat(object.search_name ? object.search_name : []),
                    { ra, dec },
                    object.vmag,
                    object.class,
                    object.image_url || null,
    ...
    static async loadNGCData(): Promise<NGCObject[][]> {
        ...
        const ic: NGCObject[] = new Array(icCount);
        ...
        const specialNgcIcData = await this.fetchJson<any[]>('data/special_ngc_ic.json');
    ...
    static async loadSharplessData(): Promise<SharplessObject[]> {
        const data = await this.fetchJson<any[]>('data/sharpless.json');
        const sharpless: SharplessObject[] = [];
        for (const object of data) {
            sharpless.push(
                new SharplessObject(
                    object.name,
                    object.alt_names,
                    object.search_names,
                    { ra: object.ra * 0.001, dec: object.dec * 0.001 },
                    object.diameter,
                    object.form,
                    object.bright,
                    object.description,
    ...
    // 明るい星データの読み込み
    static async loadBrightStars(): Promise<BayerFlamData[]> {
    ...
    // Gaiaデータの読み込み
    static async loadGaiaData(magnitudeRange: string, encodeStyle: string, magOffset: number): Promise<number[][]> {
        try {
    ...
    static async loadGaiaHelpData(magnitudeRange: string): Promise<number[]> {
    ...
    static async loadMilkyWayData(): Promise<number[][]> {
    ...
    static async loadArtemisEphemerides(): Promise<number[][]> {
        const data = await this.fetchText('data/artemisEphemerides.txt');
        const data_split = data.split('\n');
        const ephemerides: number[][] = [];
    ...
    // 追加天体データの読み込み
    static async loadAdditionalObjects(): Promise<string> {

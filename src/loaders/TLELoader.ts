import { TLEData } from '../types/index.js';

export class TLELoader {
    private static readonly CELESTRAK_URL = 'https://www.celestrak.com/NORAD/elements/';
    private static readonly SPACETRACK_URL = 'https://www.space-track.org/';

    /**
     * CelestrakからTLEデータを取得
     */
    static async loadFromCelestrak(category: string): Promise<TLEData[]> {
        try {
     ...
     * 利用可能なTLEカテゴリを取得
     */
    static getAvailableCategories(): { name: string; url: string; description: string }[] {
        return [
     ...
     * テキストからTLEデータを解析
     */
    static parseTLEFromText(text: string): TLEData[] {
        const tles: TLEData[] = [];
    ...
    private static isValidTLE(line1: string, line2: string): boolean {
        ...
        // 行1のチェック
        if (line1.charAt(0) !== '1') return false;
        
        // 行2のチェック
        if (line2.charAt(0) !== '2') return false;
     ...
     * NORAD IDを抽出
     */
    private static extractNORADId(line1: string): string | undefined {
        if (line1.length >= 7) {
     ...
     * 特定のNORAD IDのTLEを取得
     */
    static async loadSpecificSatellite(noradId: string): Promise<TLEData | null> {
        try {
            // 複数のカテゴリから検索
     ...
     * 複数のNORAD IDのTLEを取得
     */
    static async loadMultipleSatellites(noradIds: string[]): Promise<TLEData[]> {
        const tles: TLEData[] = [];
     ...
     * ローカルファイルからTLEを読み込み
     */
    static async loadFromFile(file: File): Promise<TLEData[]> {
        try {
     ...
     * TLEデータをファイルに保存
     */
    static saveToFile(tles: TLEData[], filename: string): void {
        const content = tles.map(tle => 
            `${tle.name || 'Unknown'}\n${tle.line1}\n${tle.line2}`
        ).join('\n\n');

import { SatellitePosition, CanvasSize, ViewState } from '../types/index.js';
...
export class SatelliteRenderer {
    ...
    constructor(ctx: CanvasRenderingContext2D, canvasSize: CanvasSize, viewState: ViewState, isDarkMode: boolean = false) {
        this.ctx = ctx;
     ...
     * 人工衛星を描画
     */
    renderSatellites(satellites: SatellitePosition[]): void {
     ...
     * 単一の人工衛星を描画
     */
    private renderSatellite(satellite: SatellitePosition): void {
        // 座標変換（赤道座標から画面座標）
     ...
     * 人工衛星のシンボルを描画
     */
    private drawSatelliteSymbol(x: number, y: number, size: number): void {
        // 十字マークを描画
        this.ctx.beginPath();
     ...
     * 人工衛星のラベルを描画
     */
    private drawSatelliteLabel(x: number, y: number, name: string, alt: number, range: number): void {
        const fontSize = 12;
     ...
     * 人工衛星のサイズを決定
     */
    private getSatelliteSize(magnitude: number): number {
        // 等級に基づいてサイズを決定
        if (magnitude < 3) return 4;
        if (magnitude < 5) return 3;
        if (magnitude < 7) return 2;
        return 1;
    }
     ...
     * 人工衛星の色を決定
     */
    private getSatelliteColor(illuminated: boolean): string {
     ...
     * 人工衛星の軌道を描画（将来の機能）
     */
    renderSatelliteOrbit(satellite: SatellitePosition, futurePositions: SatellitePosition[]): void {
        if (futurePositions.length < 2) return;

...
export class ColorManager {
     ...
     * @returns 色の文字列
     */
    getColor<K extends keyof ColorPalette>(key: K): ColorPalette[K] {
     ...
     * @returns 透明度付きの色
     */
    getColorWithAlpha<K extends keyof ColorPalette>(key: K, alpha: number): string {
     ...
     * @returns 人工衛星の色
     */
    getSatelliteColor(illuminated: boolean): string {
     ...
     */
    parseRgbToList(color: string): [number, number, number] {
        if (color.startsWith('rgba')) {
     ...
     */
    blendColors(colorA: [number, number, number], colorB: string, ratioB: number): string {
 ...
 * @returns 色管理インスタンス
 */
export function getColorManager(isDarkMode: boolean = false): ColorManager {

...
const greekAbbrs: { [key: string]: string } = {
 ...
 */
export function formatBayerDesignation(bayer?: string, bayerNum?: number): string {
    if (!bayer) return '';
 ...
 */
export function formatFlamsteedNumber(flam?: number): string {
    if (flam === undefined || flam <= 0) return '';

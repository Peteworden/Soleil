...
export class StarsProgram {
    ...
    draw(pointsInterleaved: Float32Array, count: number, pointScalePx: number, color: [number, number, number]): void {
        const gl = this.gl;
        ...
        if (this.uPointScaleLoc) gl.uniform1f(this.uPointScaleLoc, pointScalePx);
        //uColor: ベース色。恒星ごとの色分けをシェーダで行わない場合の一括色指定
        if (this.uColorLoc) gl.uniform3f(this.uColorLoc, color[0], color[1], color[2]);

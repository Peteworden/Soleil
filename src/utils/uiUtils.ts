...
export function handleResize() {
    ...
    if (window.innerWidth <= 768) {
        ...
        if (bottomButtons) (bottomButtons as HTMLElement).style.display = 'none';
    } else {
 ...
 * 用途: 簡単な用途、概算が必要な場合
 */
export function getAvailableScreenSize(): { width: number, height: number } {
    // フルスクリーン時は画面全体を使用
 ...
 * 用途: デバッグや詳細な分析が必要な場合
 */
export function measureBrowserUI(): { top: number, bottom: number, left: number, right: number } {
    // フルスクリーン時はUIはない
    if (document.fullscreenElement) {
 ...
 */
export function getAvailableSize(): CanvasSize {
    if (document.fullscreenElement) {
        return {
            width: window.screen.width,
            height: window.screen.height
